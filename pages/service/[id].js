...
function Services() {
  ...
  useEffect(() => {
    axios.get(`http://188.166.40.167/ru/services/${serviceId}/`).then((res) => {
      setService(res.data);
    });
  }, [serviceId]);
  ...
  return (
    <>
      <main className="main">
        <section className="services__hero" style={{backgroundImage:`url(${service.image})`}}>
          <div className="container">
            <h2 className="services__hero-title">{service.title}</h2>
          </div>
        </section>
        <section className="services__text container">
          <p className="text mb55">{service.description}</p>
        </section>
        <section className="services__team container">
          <h3 className="subtitle">Команда</h3>
          <ul className="services__list">
            {service.services_people == undefined ? (
              <>Ooops...</>
            ) : (
              service.services_people.map((item) => (
